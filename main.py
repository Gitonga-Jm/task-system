...
print("Starting Task Queue System on Render...")
...
@app.get("/")
def root():
    return {"message": "Task Queue System", "status": "running", "platform": "Render"}
...
def dashboard():
    ...
    <body>
        ...
        <p>System is running on Render!</p>
        <p>API Status: <span id="status">Checking...</span></p>
        <script>
            fetch('/health')
                .then(r => r.json())
                .then(d => document.getElementById('status').innerText = d.redis);
        </script>
    </body>
    </html>
    """)
