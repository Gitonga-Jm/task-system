...
print(f"Redis config: HOST={REDIS_HOST}, PORT={REDIS_PORT}, TLS={REDIS_TLS}")

def get_redis_client():
    """Create Redis connection with proper TLS for Upstash"""
    try:
        if REDIS_TLS:
            # Upstash requires SSL with certificate verification disabled
            client = redis.Redis(
                host=REDIS_HOST,
                port=REDIS_PORT,
